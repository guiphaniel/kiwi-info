<section class="title-group">
    <h1><?= $article['titre1'] ?></h1>
    <?php if(isset($article['titre2'])): ?>
        <h2><?= $article['titre2'] ?></h2>
    <?php endif ?>
    <p><?= $article['description']?></p>
    <em>
        Par <a href="#" ><?= $article['auteur']?></a>
        <time content="<?= $article['creationDate'] ?>" datetime="<?= $article['creationDate'] ?>">
            <span title="<?= $article['creationDate'] ?>">il y a 3 jours.</span>
        </time>
    </em>
</section>
<div id="aside-group">
    <article>
        <picture class="intro-pic">
            <img src="../img/articles-illustrations/<?=$article['image']?>" alt="Minecraft Live">
        </picture>
            ...
            <nav>
                <ul>
                    <?php
                        ...
                        <li>
                            <a class="<?php if(($_GET['article']??-1) == $row['id']) echo 'active' ?>" href="/blog.php?article=<?=$row['id']?>">
